...
async def upload_gallery_photo(photo: UploadFile = File(...)):
    ...
    try:
        ...
        # Create URL for the photo - use relative path that FastAPI will serve
...
async def get_customer_photos():
    ...
    try:
        ...
        # Method 1: Get photos from database quotes
        ...
        # Method 2: Get photos from actual file system (booking images)
        booking_images_dir = "/app/backend/static/booking_images"
        if os.path.exists(booking_images_dir):
            for filename in os.listdir(booking_images_dir):
                if filename.lower().endswith(('.jpg', '.jpeg', '.png', '.webp')):
                    file_path = os.path.join(booking_images_dir, filename)
                    file_stat = os.stat(file_path)
                    
                    # Extract booking ID from filename if possible
                    booking_id = ""
                    if filename.startswith("booking_"):
                        booking_id = filename.split("_")[1] if "_" in filename else ""
                    
                    customer_photos.append({
                        "url": f"/static/booking_images/{filename}",
                        "quote_id": booking_id,
                        "uploaded_at": datetime.fromtimestamp(file_stat.st_mtime).isoformat(),
                        "description": f"Customer photo: {filename}",
                        "source": "filesystem",
                        "file_size": file_stat.st_size
                    })
        
        # Method 3: Get completion photos as well
        completion_photos_dir = "/app/backend/static/completion_photos"
        if os.path.exists(completion_photos_dir):
            for filename in os.listdir(completion_photos_dir):
                if filename.lower().endswith(('.jpg', '.jpeg', '.png', '.webp')):
                    ...
                    # Extract booking ID from filename if possible
                    booking_id = ""
                    if filename.startswith("completion_"):
                        booking_id = filename.split("_")[1] if "_" in filename else ""
                    
                    customer_photos.append({
                        "url": f"/static/completion_photos/{filename}",
                        "quote_id": booking_id,
                        "uploaded_at": datetime.fromtimestamp(file_stat.st_mtime).isoformat(),
                        "description": f"Completion photo: {filename}",
                        "source": "completion",
                        "file_size": file_stat.st_size
                    })
